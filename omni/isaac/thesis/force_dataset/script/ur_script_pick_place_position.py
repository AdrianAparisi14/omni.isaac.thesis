...
PLACE_APPROX_POSE = [0.2863001578683557, 0.36359806537224587, 0.335, 2.897516032346434, -1.2120822261580186, 0.002188515010319512]
PLACE_APPROX_Q = [1.1950923204421997, -1.52761264265094, -2.031050205230713, -1.1723521512797852, 1.5695853233337402, 0.41885924339294434]

PICK_POSE = [0.5105244047089075, 0.13011966784806409, 0.2278826285091872, 2.897222744351678, -1.2125884065595494, 0.0024652353585882325]
PICK_Q = [0.5050216913223267, -1.8198548756041468, -2.0205655097961426, -0.889411525135376, 1.568636417388916, -0.2712319532977503]
...
def get_hole_target(pose, orientation, timestep):
    ...
    if hole_target_pose[2] < 0.0385:   #-0.0004: #Correct assembly: 0.0033 // stop early: 0.0105 // stop late: -0.00035
...
def main():
    ...
    time.sleep(0.1)
    ur_robot.setInputIntRegister(19, recipe) # Release position of the gripper
    time.sleep(0.5)
    ...
    csv_file_path = "../../../../../Documents/sdu.extensions.2023.1.1/exts/omni.isaac.thesis/omni/isaac/thesis/force_dataset/csv_real_robot_sdu/csv_real_robot_position/data" + str(current_time) + ".csv"
