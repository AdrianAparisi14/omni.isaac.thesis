...
def get_hole_target(pose, orientation, timestep):
    ...
    if hole_target_pose[2] < 0.0033:   #-0.0004: #Correct assembly: 0.0033 // stop early: 0.0105 // stop late: -0.00035
...
def force_compensation(Fg, Foff, Moff, r, F, R1):
    Fin = F[:3]
    Min = F[3:]
...
def main():
    ...
    # #Gripper force compensation variables loading:
    ...
    # Fg = np.array(data["Fg"])
    # Foff = np.array(data["Foff"])
    # Moff = np.array(data["Moff"])
    # r = np.array(data["r"])
