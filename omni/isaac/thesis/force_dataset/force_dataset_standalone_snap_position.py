...
simulation_context.set_simulation_dt(physics_dt=1/80.0, rendering_dt=1/120.0)
...
class dataset_generation():
    ...
    def create_scene(self, world, tool, mode):
    ...
    def init_step(self):
        ...
        if self.tool == "snap_tool_001_flat.usd" or self.tool == None:
            init_target = XFormPrim("/World/"+ snap_bottom +"/Snap_Bottom_top").get_world_pose()[0]
        rot = None
        rot = r.euler_angles_to_quat([180,0,0],degrees=True)
        self.robot.movePose3(ut.get_pose3(np.add(init_target,[random.uniform(-0.0001, 0.0001),random.uniform(-0.0001, 0.0001),0.2865]),rot_quat=rot))
    ...
    def get_hole_target(self, pose, orientation, timestep):
        ...
        if self.mode == 2:
            if hole_target_pose[2] < 0.0007: # Correct assembly: 0.0033 // Stop early: 0.0115 // Stop late: 0.0007
                hole_target_pose[2] = 0.0007
        else:
            if hole_target_pose[2] < 0.0033: # Correct assembly: 0.0033 // Stop early: 0.0115 // Stop late: 0.0007
    ...
    def physics_step(self):
        ...
        wrist_forces_ = np.array([-wrist_forces[0],-wrist_forces[1],-wrist_forces[2]+noise,-wrist_forces[3],-wrist_forces[4],-wrist_forces[5]])
...
def parse_arguments():
    ...
    parser.add_argument('--save_values', type=str_to_bool, default=False, help='Whether to save values (True/False)')
    parser.add_argument('--mode', type=int, default=1, help='Mode (integer) 1 (success); 2 (fail); 3 (fail) ')
...
# Create the scene
dataset.create_scene(world, tool_tip, mode)
    ...
    try:
        ...
        if save_values:
            ...
            if mode == 1:
                ...
                result = '0'
            print("Result of the assembly: ", result) 
            ...
            ref_timeseries_path = "../../../../../Documents/sdu.extensions.2023.1.1/exts/omni.isaac.thesis/omni/isaac/thesis/force_dataset/csv_real_robot_sdu/csv_real_robot_position_all_correct/data2024-04-25 08:31:42.286459.csv"
